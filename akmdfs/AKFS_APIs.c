...
/*! Initialize #AKMPRMS structure and make APIs ready to use. This function
  ...
  @param[in] accOffset[3] Specify the accelerometer offset
  @param[in] accCodePerG Specify the accelerometer sensitivity, code/g
*/
int16 AKFS_Init(
		void		*mem,
		const	AKFS_PATNO	hpat,
		const	uint8		regs[],
		const   AKFLOAT	        accOffset[],
		const   AKFLOAT         accCodePerG
		)
{
  AKMPRMS *prms;
  ...
  prms->fv_as.u.x = accCodePerG;
  prms->fv_as.u.y = accCodePerG;
  prms->fv_as.u.z = accCodePerG;
  ...
  /* Copy the offset values */
  prms->fv_ao.u.x = accOffset[0];
  prms->fv_ao.u.y = accOffset[1];
  prms->fv_ao.u.z = accOffset[2];
